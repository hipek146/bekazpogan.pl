...
if(isset($_POST['stars']) && !empty($_POST['stars']) && isset($_POST['id']) && !empty($_POST['id']))
{
	...
	$query = "SELECT id, stars, rating, s1, s2, s3, s4, s5 FROM posts WHERE id = " . $_POST['id'];
	...
	mysqli_query($connection, "UPDATE posts SET ". $_POST['stars'] ."=" . ($r[$_POST['stars']] + 1) . "
    WHERE id='$id'");
	$i = $r['s1']+$r['s2']+$r['s3']+$r['s4']+$r['s5']+1;
	mysqli_query($connection, "UPDATE posts SET rating=" . $i . "
    WHERE id='$id'");
	$k = ($r['s1']*1 + $r['s2']*2 + $r['s3']*3 + $r['s4']*4 + $r['s5']*5 + $s)/$i;
	mysqli_query($connection, "UPDATE posts SET stars=" . round_up($k, 4) . "
    WHERE id='$id'");
	echo $k;
	}
