...
$query = "SELECT id, title, host, stars, type FROM poczekalnia  WHERE id = " . $id;
$total = @mysqli_num_rows($data); 
$result = mysqli_query($connection, $query);
while($r = @mysqli_fetch_assoc($result)) { 
echo '<meta property="og:url"                content="http://bekazpogan.pl/temp/'. $id .'" />';
echo '<meta property="og:title"              content="' . $r["title"] . '" />';
echo '<meta property="og:image"              content="http://bekazpogan.pl/' . substr($r["host"], 3) . '" />';
}
?>
...
echo '<div class="fb-comments" data-colorscheme="dark" data-href="http://bekazpogan.pl/temp/' . $id . '" data-numposts="5"></div>';
?>
