...
<script>
  (adsbygoogle = window.adsbygoogle || []).push({
    google_ad_client: "ca-pub-5529160104147863",
...
<meta charset="UTF-8">
  <meta name="description" content="Najlepszy humor w całym internecie! Śmieszki, świry i wiele więcej!">
  <meta name="keywords" content="beka, beka z pogan, poganie, memy, humor, śmieszne, dowcipy, zabawa">
...
function generate_page_links($cur_page, $num_pages) {
     ...
     if ($cur_page > 1) {
          $before= '<a href="/page/' . ($cur_page - 1) . '" class = "pager2">Poprzednia</a> ';
     }
          ...
          if ($i > 0 && $i <= $num_pages) {
               ...
               if ($cur_page == $i  && $i != 0) {
                    ...
                    if ($i == ($cur_page - 4) && ($cur_page - 5) != 0) { 
                         $page_links .= ' <a href="' . '/page/1" class = "pager">1</a> '; 
                    }
               
                    //wyświetlamy "kropki", jako odnośnik do poprzedniego bloku stron
                    if ($i == ($cur_page - 4) && (($cur_page - 6)) > 0) { 
                         $page_links .= ' <a href="' . '/page/' . ($cur_page - 5) . '" class = "pager">...</a> '; 
                    } 
               
                    //wyświetlamy liki do bieżących stron
                    $page_links .= ' <a href="' . '/page/' . $i . '" class = "pager"> ' . $i . '</a> ';
                    ...
                    if ($i == $page && ($cur_page + 4) != $num_pages) { 
                         $page_links .= ' <a href="' . '/page/' . $num_pages . '" class = "pager">' . $num_pages . '</a> '; 
                    }
               }
          }
     }
     ...
     if ($cur_page < $num_pages) {
          $next = '<a href="' . '/page/' . ($cur_page + 1) . '" class = "pager2">Następna</a>';
     }
...
$bol = 1;	
	...
	while($r = @mysqli_fetch_assoc($result)) {
		if($post_nr == 2)include("ad1.php");
		if($post_nr == 4)include("ad2.php");
		$post_nr= $post_nr +1;		
		$s = $r['stars'];
		$ss = $s;
		$d = $r['id'];
...
$bol = 0;
		if($r['type'] == "img")
			echo "<a href = \"../post/" . $d . "\"><img src=\"" . $r['host'] . "\" width = 85% style = \"border: 2px solid black; margin-bottom: 10px;\"/></a><br>";
		else if($r['type'] == "txt")
			echo "<a class=\"txt1\" href = \"../post/" . $d . "\"><div class = \"txt\" style = \"border: 2px solid black; margin-bottom: 10px; width: 85%; background: black;\">" . nl2br($r['host']). "</div></a><br>";
		else if($r['type'] == "yt")
		...
		echo "<table width = 95% cellpadding=\"0\" cellspacing=\"0\"><tr><td <tr align=\"right\" width = 100%><table cellpadding=\"0\" cellspacing=\"5\" style=\"margin-bottom: 100px;\"><tr><td>";
		echo "<div class=\"fb-share-button\" data-href=\"../post/".$d."\" data-layout=\"button\" data-size=\"large\" data-mobile-iframe=\"true\"><a class=\"fb-xfbml-parse-ignore\" target=\"_blank\" href=\"https://www.facebook.com/sharer/sharer.php?u=http%3A%2F%2Fbekazpogan.pl%2F&amp;src=sdkpreparse\">Udostępnij</a></div>";
	...
	padding-right: 15px;
		...
		echo "<td><div id =\"lnk" . $r['id'] . "\" class = \"lnk\" onmouseover = \"com_ov(this);\" onmouseout = \"com_ou(this);\" onclick  = \"cop_c(this, 'www.bekazpogan.pl/post/" . $r['id'] . "');\" data-clipboard-text='www.bekazpogan.pl/post/" . $r['id'] . "'>KOPIUJ LINK</div></td></tr></table></td></tr></table>";
